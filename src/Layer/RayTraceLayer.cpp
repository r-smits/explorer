...
EXP::RayTraceLayer::RayTraceLayer(MTL::Device* device, std::shared_ptr<const AppProperties> _config)
	...
	_outputTexture = device->newTexture(desc);
	_outputTexture->retain();
	desc->release();
	
	buildModels(device);
	buildAccelerationStructures(device);
	initialize_imgui(device);
	DEBUG("Initialized Ray trace layer");
}

EXP::RayTraceLayer::~RayTraceLayer() {
    device->release();
    queue->release();
...
void EXP::RayTraceLayer::onUpdate(MTK::View* view, MTL::RenderCommandEncoder* notUsed) {
	...
	// temporalEncoder->setTexture(_outputTexture, 0);
